...
Overall the \ac{SSSA} is designed for less resource consumption and more efficient context use. The architecture is mainly designed towards context limits, that can apply, while keeping the idea of iterating over the process. In addition, it is another approach to split up the supervisor agent and therefore remains an architecture following the custom multi-agent system pattern.
...
\begin{itemize}
  \item Idea: Test simplest solution for cost and time efficiency and Reduce Steps to save context size
  \item Simpler Optimisation (Main Agent has to work better with the tools)
  \item State
  \item Tools needed to accomplish task (URI Retrieval)
  \item Additional Tools as of good performance
\end{itemize}
...
\begin{itemize}
  \item Idea: Leverage Ideas and Capabilities of ReAct Agent, but combined with validation capabilities from Gen1. Leads to build less specialised but fully connected agents. Leverage each agent in fact of routing and reasoning.
  \item State
  \item Agents used: Extractor -> like in ReAct Agent, URI Retrieval,
  \item Tools
\end{itemize}
...
\begin{itemize}
  \item VectorDB, multiple collections (Label, Description, Sample (Properties Only))
  \item Search Modes (No Mode -> Searching for all on Label, Select for Collection, Filter for Entity/Property)
  \item Latest: Entity, Property, Sample
  \item LLM Incorporation in baseline
\end{itemize}

In order to limit the context length, the tool was extended by a LLM call, where the LLM was asked to format the search results and create a primliminary mapping of the search terms to the URIs. Later extended by access to more parts of the state in i.e. Gen1v2.
...
\begin{itemize}
  ...
  \item Find Edge Cases and add them to Guidelines Section
\end{itemize}

\section{Error Incorporation}
\label{sec:error_incorportion}
\begin{itemize}
  \item Validation of Turtle output
  \item Check Agent Flow with Error Loops -> Prevent Agent System from throwing any error and aborting the whole execution
\end{itemize}
