...
\begin{itemize}
  \item Novel Approach using MAS for cIE
  \item Leverage tools among knowledge graph incorporation
  \item Designed to be general-usable instead of fine-tuned to one dataset to match real-world performance
  \item Concept after \cite{Brown2020} to just use in-context learning, as also LLMs has advanced (reference on section in experimental setup)
  \item Iterative Approach based on Agent Design ideas
  \item That is why four architectures emerged: Baseline, Supervisor, ReAct, Network
  \item Five tools emerged: URI Search, Network Traversal, Message Deletion, Semantic Validation, Turtle to Label
\end{itemize}
