...
\begin{table}[h]
  ...
  \begin{tabular}{p{5cm}|rrrrrr}
    ...
    ReAct Architecture          & 0.343                                         & 0.296 & 0.312 & 0.385   & 0.395   & 0.120 \\
    URI Retrieval (SSSA)        & 0.325                                         & 0.305 & 0.308 & 0.337   & 0.337   & 0.060 \\
    URI Retrieval                                                                                                           \\(ReAct) & 0.350 & 0.285 & 0.305 & 0.354 & 0.364 & 0.000 \\
...
With this simple approach, 97\% of the initial baseline performance could be reached with the achieved triple F1 of 0.312. In addition, the ReAct agent could lower the token use in contrast to the updated baseline with error incorporation by nearly 11\%, as for the benchmark 670 thousand tokens instead of 753 thousand tokens were used. While being more efficient, the ReAct agent was more simple to tune, as there was a single point to do it. This also added complexity as the capability of the used Llama 3.3 70B showed high limitation in instruction following, being hard to tune towards the goal.

In all categories the ReAct agent showed a larger gap between precision and recall, with precision being higher than recall. This is unlike the earlier models and shows that the ReAct agent was focussed on a few but therefore correct triples. Unlike the \ac{SSSA} fewer triples will be generated overall.

Nevertheless, the ReAct agent a showed new high-scores among the previous models in the F1 of extracting subjects. Other than that the ReAct agent showed scores in the remaining categories that were matching the one of the \ac{SSSA} implementation. In addition the error rate is 12\% instead of 4\% like in the \ac{SSSA} what also goes back to context window issues. This implies, that the ReAct architecture has a theoretical potential of scoring 0.354, if the scores scale proportianal.

An result limiting factor is the missing sense for the usage of the URIs like in all previous models. As an example the sentence \enquote{Herman Heijermans was a citizen of the Kingdom of the Netherlands.} expected the property \enquote{country of citizenship}.

\textbf{ERROR EXPLANATION}
...
The adaption had positive results for the \ac{SSSA}, while the ReAct solution was achieving similar results than before. The implementation \ac{SSSA} had a slightly higher error rate of 6\%, while achieving XY\% better results with a F1-score of XYZ than the original \ac{SSSA} implementation. On the other side the ReAct agent had achieved a 0\% error rate, but nevertheless the F1-score decreased to XYZ. Therefore, the \ac{SSSA}, ReAct agent and various baseline implementation are scoring close to each other.

When taking a look into the seperate parts of the evaluation, it get visible, that the ReAct agent with the new filtering mode can score up top or nearly top of the prior models. Espeically subjects and entities overall get very good extracted by the ReAct agent with F1-score of XYZ and XYZ. On the property and object category it is defeated by the baseline architecture with error incorporation. In addition, the updated \ac{SSSA} is also able to score nearly as good as the ReAct agent, explaining the improved F1-scores on the triple category. The property F1-scores are lowered  for the \ac{SSSA} by XY\%, while being increase for the ReAct agent by XY\%, which shows that the URI-filtering had no high effect.

As the extraction of subject, objects is already nearly perfect, the biggest limiting factor remains the extraction of properties and the merging of all parts into correct triples. In traces it is shown, that... \textbf{ERROR EXPLANATION - URI Retrieval doesn't found}. In addition, the soft matches show high potential in achieve higher triple F1-scores, with the parental F1 being XY\% higher than the normal F1, which goes also for properties.

All in all the URI retrieval filtering reveals a high potential in extracting correct triples. The ReAct agent achieves near perfect extraction for subject and entities with scores above XYZ F1. Also the \ac{SSSA} achieves scores lower, but also similar to the ReAct agent. Nevertheless, the problem is, that the high scores in those categories are not reflected in the triple F1-scores. In addition, the URI-filtering does not have shown an effect on the property or triple F1 score.
...
The property extraction, no matter whether it was done by a separate agent or not, for instance opened up a gap of maximum XYZ\% worse than on the subject category. Therefore, the expectation is, that if the properties are correct, but just soft matches, the correct ones can be found within the property hierarchy of Wikidata for the case of synthIE. In addition, this hierarchy may also give an intend of the semantics of the original property.
...
As the ReAct archiecture remained simpler to develop, it was the architecture of choice to test the knowledge graph integration with the network traversal tool. First of all, the result is that when the agent evaluates a property as not matching, it starts to use the network traversal tool. As example \textbf{INSERT EXAMPLE FROM LANGFUSE}

In the end, this does not lead to the expected effect of higher triple F1-scores. Instead the knowledge graph integration lowers the F1 by XYZ\%, achieving XYZ F1. This goes also for all remaining categories. Nevertheless, two effects are apparent in the detailed evaluation. First, the scores on all categories do show a high gap to the top previous models, but the F1-score is not as affected as it could be expected. Second, the error rate is at 22\% and shows the problem of context overflowing, which is mainly caused by the agent trying to use the URI search tool with various search terms and modes. The error rate shows a much higher potential F1 of XYZ\%.
...
\begin{itemize}
  ...
  \item Context Window Issues
  \item Limitation of Wikidata on Relevant Results -> Simplicity of switching datasets, Time efficiency, Performance
  \item No Limitation of Outputs like in synthIE -> Therefore, very adaptable
\end{itemize}
...
\begin{table}[h]
  ...
  \begin{tabular}{p{6cm}|rrrr}
    ...
    ReAct Architecture                   & 0.813                                          & 0.738          & 0.751          & 0.120          \\
    URI Retrieval (SSSA)                 & 0.800                                          & 0.818          & 0.777          & 0.060          \\
    URI Retrieval (ReAct)                & 0.910                                          & 0.848          & 0.848          & 0.000          \\
...
\begin{table}[h]
  ...
  \begin{tabular}{p{4cm}|rrrrrr}
    ...
    ReAct Architecture                   & 0.418                                            & 0.351          & 0.374          & 0.455          & 0.470          & 0.120          \\
    URI Retrieval (SSSA)                 & 0.396                                            & 0.334          & 0.355          & 0.438          & 0.442          & 0.060          \\
    URI Retrieval (ReAct)                & 0.485                                            & 0.367          & 0.400          & 0.480          & 0.496          & 0.000          \\
...
\begin{table}[h]
  ...
  \begin{tabular}{p{6cm}|rrrr}
    ...
    ReAct Architecture                   & 0.793                                         & 0.664          & 0.708          & 0.120          \\
    URI Retrieval (SSSA)                 & 0.789                                         & 0.750          & 0.746          & 0.060          \\
    URI Retrieval (ReAct)                & 0.848                                         & 0.665          & 0.719          & 0.000          \\
...
\begin{table}[h]
  ...
  \begin{tabular}{p{6cm}|rrrr}
    ...
    ReAct Architecture                   & 0.835                                          & 0.747          & 0.781          & 0.120          \\
    URI Retrieval (SSSA)                 & 0.868                                          & 0.810          & 0.829          & 0.060          \\
    URI Retrieval (ReAct)                & 0.935                                          & 0.792          & 0.840          & 0.000          \\
